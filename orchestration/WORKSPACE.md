...
- **`detect.mjs`** — runs the detection algorithm; emits a `workspace@2` JSON object
...
- **Overrides** (`overrides.local.json`) are the **only durable, human-authored
  artifact**: role/gate/knowledge decisions the user made that must outlive
...
unavailable). The script emits a `workspace@2` object with `decisions_needed`
...
The profile is **`workspace.json`** — schema `orchestration/workspace@2`, the exact
...
```jsonc
{
  "schema": "orchestration/workspace@2",
  "generated": "<ISO-8601>",
...
rendered `.md` derives `Active roles`, `Per-case handling` (from member `notes` +
flagged roles), and `How to run` automatically — they are not stored.
