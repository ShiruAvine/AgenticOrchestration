...
2. **Establish the workspace (Phase 0).** Load the personal, gitignored workspace profile — read **`workspace.json`** (the source of truth, always under `./.claude/orchestration/`: `workspace.json` for multi-repo, or `workspace.local.json` for single-repo / monorepo), not the rendered `.md`. **If none exists, run `/orchestrate-setup` first** — it dispatches `chuck-workspace-analyst` to detect the topology (single-repo / monorepo / multi-repo parent folder) deterministically, asks you only the crucial decisions, and writes the profile. See `${CLAUDE_PLUGIN_ROOT}/WORKSPACE.md`.
