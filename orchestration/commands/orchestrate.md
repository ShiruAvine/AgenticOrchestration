...
4. Read each active member's `CLAUDE.md` to bind the generic specialists to that member's domain map (which directories hold what, which convention checks apply, where skills live); use that member's gate commands from the profile.
